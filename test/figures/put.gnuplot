...
fit Tc(x) 'put.dat' using 1:2 via alpha, beta
set title sprintf("Epiphany-III OpenSHMEM PutMem Performance\n{/*0.9 α = %.0f ± %.0f nsec, β^{-1} = %.3f ± %.3f GB/s}", alpha, alpha_err, 1/beta, beta_err/beta)
...
plot "put.dat" using 1:(PEs*$1/$2) title 'Bandwidth' with linespoints axes x2y2, \
 "put.dat" using 1:($2/1000) title 'Latency' with linespoints lt 2 lc rgb "blue"
